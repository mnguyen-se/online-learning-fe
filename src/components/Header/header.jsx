...
const Header = () => {
  ...
  const handleLookupKanji = async () => {
    ...
    try {
      ...
      toast.error(
        error.message || "Có lỗi xảy ra khi tra cứu kanji. Vui lòng thử lại.",
      );
      setKanjiInfo(null);
    } finally {
                      ...
                      <div className="kanji-result">
                        <div className="kanji-display">
                          ...
                          <div className="kanji-badges">
                            {kanjiInfo.jlpt && (
                              <span className="kanji-jlpt">
                                JLPT N{kanjiInfo.jlpt}
                              </span>
                            )}
                            {kanjiInfo.grade && (
                              <span className="kanji-grade">
                                Lớp {kanjiInfo.grade}
                              </span>
                            )}
                            {kanjiInfo.strokeCount && (
                              <span className="kanji-strokes">
                                {kanjiInfo.strokeCount} nét
                              </span>
                            )}
                          </div>
                        </div>
                        <div className="kanji-details">
                          {kanjiInfo.meaningsVi &&
                            kanjiInfo.meaningsVi.length > 0 && (
                              <div className="kanji-section">
                                <h4>Nghĩa (Tiếng Việt):</h4>
                                <div className="kanji-meanings">
                                  {kanjiInfo.meaningsVi.map(
                                    (meaning, index) => (
                                      <span
                                        key={index}
                                        className="kanji-meaning vi-meaning"
                                      >
                                        {meaning}
                                      </span>
                                    ),
                                ...
                                {kanjiInfo.meanings &&
                                  kanjiInfo.meanings.length > 0 && (
                                    <div className="kanji-meanings-en">
                                      ...
                                      {kanjiInfo.meanings.map(
                                        (meaning, index) => (
                                          <span key={index}>
                                            <span className="kanji-meaning-en">
                                              {meaning}
                                            </span>
                                            {index <
                                              kanjiInfo.meanings.length - 1 && (
                                              <span>, </span>
                                            )}
                                          </span>
                                        ),
                              ...
                              <div className="kanji-section">
                                ...
                                <div className="kanji-meanings">
                                  {kanjiInfo.meanings.map((meaning, index) => (
                                    <span key={index} className="kanji-meaning">
                                      {meaning}
                                    </span>
                                  ))}
                                </div>
                          ...
                          {kanjiInfo.readings.onyomi &&
                            kanjiInfo.readings.onyomi.length > 0 && (
                              <div className="kanji-section">
                                <h4>Âm On (音読み):</h4>
                                <div className="kanji-readings">
                                  {kanjiInfo.readings.onyomi.map(
                                    (reading, index) => (
                                      <span
                                        key={index}
                                        className="kanji-reading onyomi"
                                      >
                                        {reading}
                                      </span>
                                    ),
                          ...
                          {kanjiInfo.readings.kunyomi &&
                            kanjiInfo.readings.kunyomi.length > 0 && (
                              <div className="kanji-section">
                                <h4>Âm Kun (訓読み):</h4>
                                <div className="kanji-readings">
                                  {kanjiInfo.readings.kunyomi.map(
                                    (reading, index) => (
                                      <span
                                        key={index}
                                        className="kanji-reading kunyomi"
                                      >
                                        {reading}
                                      </span>
                                    ),
